...
export async function generateMetadata({ params }: Props) {
  const page = INDUSTRY_FINANCE_PAGES.find((p) => p.slug === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/industry-finance/${page.slug}` },
    openGraph: {
      title: `${page.title} | Nexa Consultancy`,
      description: page.description,
      url: `/industry-finance/${page.slug}`,
      type: 'article',
    },
  };
}

export default function IndustryFinancePage({ params }: Props) {
  const { slug } = params;
  const page = INDUSTRY_FINANCE_PAGES.find((p) => p.slug === params.slug);
        ...
        <div className="grid md:grid-cols-3 gap-12 py-16 md:py-24">
            <div className="md:col-span-2">
                <div className="prose lg:prose-lg max-w-none">
                    ...
                    <h3 className="text-2xl font-bold text-primary mt-12 mb-4">{page.keyConsiderations.title}</h3>
                    <ul className="space-y-3 !pl-0">
                        {page.keyConsiderations.points.map(point => (
